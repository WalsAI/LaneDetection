...
from LaneDetection.Data.utilities.utils import nd_array_to_torch_float_channels_first
...
class RoadMarkingDataloader(pl.LightningDataModule):
    ...
    '''
    def __init__(self, train_csv:str, val_csv:str, test_csv:str,batch_size:int , image_column:str, mask_column:str, mean: List[float], std: List[float]):
        '''
        ...
        self.__transform =  T.Compose([
            nd_array_to_torch_float_channels_first,
            T.Normalize(mean=mean, std=std)
        ])

        self.__gt_transform =  T.Compose([
            nd_array_to_torch_float_channels_first,
        ])
    ...
    def setup(self, stage: str):
        ...
        '''
        if stage == 'fit':
            self.stage_ds['train'] = RoadMarkingDataset(self.train_csv, self.image_column, self.mask_column, self.__transform, self.__gt_transform)
            self.stage_ds['val'] = RoadMarkingDataset(self.val_csv, self.image_column, self.mask_column, self.__transform, self.__gt_transform)
        if stage == 'test':
            self.stage_ds['test'] = RoadMarkingDataset(self.test_csv, self.image_column, self.mask_column, self.__transform, self.__gt_transform)
